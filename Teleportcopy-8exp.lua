...
-- 1. 빠른 검색을 위한 Lookup 테이블 생성 (루프 밖에서 실행)
local expLookup = {}
for _, info in ipairs(expTable) do
    expLookup[info.category] = info
end

-- 2. 배치 설정값 (글자 공간을 위해 RowHeight를 65로 조정 추천)
local expIconSize = 40       -- 확장팩 로고 크기
local expIconX = 20          -- 왼쪽 여백
local expIconY = -20         -- 상단 여백
local expIconRowHeight = 60  -- 아이콘 + 텍스트 공간 확보를 위해 늘림

local currentCategory = ""
local row = -1 

for i, data in ipairs(teleportTable) do
    -- [섹션 처리] 새로운 카테고리가 등장할 때만 실행
    if data.category ~= currentCategory then
        row = row + 1
        currentCategory = data.category

        -- [A] 데이터 준비 (expTable에서 가져오기)
        local expinfo = expLookup[data.category]
        local rawExpName = tostring(expinfo and expinfo.name or data.category or "Unknown")
        local iconID = expinfo and expinfo.iconID or 132311

        -- [B] 확장팩 아이콘(Texture) 생성
        local expTexture = TeleportBackground:CreateTexture(nil, "ARTWORK")
        expTexture:SetSize(expIconSize, expIconSize)
        
        local yPos = expIconY - (row * expIconRowHeight)
        expTexture:SetPoint("TOPLEFT", TeleportBackground, "TOPLEFT", expIconX, yPos)
        expTexture:SetTexture(iconID)

        -- [C] 확장팩 이름(FontString) 생성
        local expNameText = TeleportBackground:CreateFontString(nil, "OVERLAY", "SystemFont_Outline_Small")
        expNameText:SetDrawLayer("OVERLAY", 7)
        expNameText:SetIgnoreParentAlpha(true)
        expNameText:SetPoint("TOP", expTexture, "BOTTOM", 0, 3) -- 아이콘 아래로 살짝 내림
        expNameText:SetWidth(expIconSize + 30)
        expNameText:SetJustifyH("CENTER")
        expNameText:SetWordWrap(false)

        -- [D] 글자 수에 따른 폰트 크기 조절 (expTable의 이름 기준)
        local charCount = strlenutf8(rawExpName)
        local fontSize = 12
        if charCount >= 4 then
            fontSize = 10 -- "내부 전쟁"(4글자) 등은 작게
        end

        local fontPath = expNameText:GetFont()
        expNameText:SetFont(fontPath, fontSize, "OUTLINE")
        expNameText:SetShadowColor(0, 0, 0, 1)
        expNameText:SetShadowOffset(1, -1)
        
        expNameText:SetText(rawExpName)
        expNameText:SetTextColor(1, 0.82, 0) -- 황금색
    ...
    -- 이 아래에 주문 버튼(TeleBtn) 배치 코드가 이어집니다.
end
