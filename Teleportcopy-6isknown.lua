...
for i, data in ipairs(teleportTable) do
 ...
 -- [수집/학습 여부 판별 로직]
    ...
    if data.type == "spell" then
        isKnown = IsSpellKnown(data.id)
    elseif data.type == "item" then
        isKnown = PlayerHasToy(data.id) or (C_Item.GetItemCount(data.id) > 0)
    else -- type == "icon"
        isKnown = true
    end
    ...
    if data.type == "spell" then
        ...
        btn:SetAttribute("spell", data.id)
    else
        btn:SetAttribute("type", "item")
        btn:SetAttribute("item", "item:"..data.id) -- ID 형식으로 언어 문제 방지
    end
